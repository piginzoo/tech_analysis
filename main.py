...
def block_zx_level2_pattern(args):

    # price = load_data(code)
    price = load_stock(args.code)
    start = str2date(args.start_date)
    end = str2date(args.end_date)
    price = price[(price.index>start)&(price.index<end)]

    price['symbol'] = price.code
...
# python main.py -c 600196 -s 20210101 -e 20230410
